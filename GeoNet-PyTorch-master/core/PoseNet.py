...
if __name__ == '__main__':
    ...
    model = PoseNet(2)
    #im = np.random.randn(1, 9, 512, 512)

    im1 = np.random.randn(1, 3, 512, 512)

    im2= np.random.randn(1, 3, 512, 512)

    im3 = np.random.randn(1, 3, 512, 512)

    im1 = torch.tensor(im1)
    im2 = torch.tensor(im2)
    im3 = torch.tensor(im3)

    im = torch.cat((im1,im2,im3),dim=1)
    im_variable = Variable(im).float()
    pose = model(im_variable)
    print(pose.shape)
